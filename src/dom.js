...
const domHandler = (() => {
  ...
  function playerPlacement(playerBoard) {
    ...
    domHandler.displayConsole(
      "Welcome to Battleship! Start the game by placing your carrier on the player board"
    );
    ...
    playerCells.forEach(function (playerCell) {
      playerCell.addEventListener("click", function (e) {
        if (shipArray.length > 0) {
          ...
          if (
            playerBoard.checkPlacementLegality(
              shipArray[0],
              direction,
            ...
            shipArray.shift();
            if (shipArray.length > 0) {
              domHandler.displayConsole(`Place your ${shipArray[0].name}`);
            } else {
              domHandler.displayConsole(
                "All ships placed! Continue by guessing on the NPC board"
              );
              gameStart = true;
            }
          } else {
            domHandler.displayConsole("choose a legal placement");
          }
        }
  ...
  function generateBoards(board1, board2) {
    ...
    for (const coordinates of board2.constructBoard()) {
      ...
      cell.addEventListener("click", function (e) {
        ...
        if (player1.getTurn() && checkLegal(e)) {
          player1.attack(
            e,
            parseInt(data[0]),
            parseInt(data[2]),
